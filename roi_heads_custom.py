...
def IoU_balanced_loss(class_logits, labels, box_regression, regression_targets):
    sampled_pos_inds_subset = torch.where(labels > 0)[0]
    pos_labels = labels[sampled_pos_inds_subset]
    sampled_neg_inds_subset = torch.where(labels <= 0)[0]
    neg_labels = labels[sampled_neg_inds_subset]
    ...
    for i in range(len(labels)):
        if labels[i].item() > 0:
            print('a')
            pos_label_indexes.append(i)

    pos_class_logits_list = []
    neg_class_logits_list = []

    for i in range(len(class_logits)):
        if i in pos_label_indexes:
            print('b')
            pos_class_logits_list.append(class_logits[i])
        else:
            neg_class_logits_list.append(class_logits[i])

    if not pos_class_logits_list:
        pos_class_logits_list.append(torch.zeros(4))
        pos_labels = torch.tensor([0.00000000], dtype=torch.long)

    print('LOGIT')
    print(pos_class_logits_list)
    pos_class_logits = torch.stack(pos_class_logits_list)
    neg_class_logits = torch.stack(neg_class_logits_list)

    box_predictions = box_regression[sampled_pos_inds_subset, pos_labels]
    box_targets = regression_targets[sampled_pos_inds_subset]

    iou_pos_part = jaccard(box_predictions, box_targets)
    # print('IOU_POS_PART')
    # print(iou_pos_part)

    cross_pos_part = F.cross_entropy(pos_class_logits, pos_labels)
    # print('CROSS_POS_PART')
    # print(cross_pos_part)
    cross_neg_part = F.cross_entropy(neg_class_logits, neg_labels)
    # print('CROSS_NEG_PART')
    # print(cross_neg_part)
    w_pos_part = iou_pos_part * ((cross_pos_part + 1) / (iou_pos_part * cross_pos_part + 1))
    # print('W_POS_PART')
    # print(w_pos_part)
    iou_balanced_loss = w_pos_part * cross_pos_part + cross_neg_part
    return torch.mean(iou_balanced_loss)
...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets):
    ...
    # REPLACE THIS FUNCTION WITH IoU-BALANCED CLASSIFICATION LOSS FUNCTION
    # print('LOSS')
    # classification_loss = F.cross_entropy(class_logits, labels)
    # print(classification_loss)
    classification_loss = IoU_balanced_loss(class_logits, labels, box_regression, regression_targets)
    # print(iou_loss)
